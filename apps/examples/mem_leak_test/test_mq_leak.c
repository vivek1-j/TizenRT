...
void test_mq_leak_after_close(void)
{
	...
	printf("  │                                                       │\n");
	printf("  │ Current tool: MAY MISS (false negative possible)     │\n");
	printf("  │   Why? After mq_close(), the MQ message buffer is      │\n");
	printf("  │   freed but may still contain stale pointer data.      │\n");
	printf("  │   heap_check() scans ALL heap blocks as roots, so      │\n");
	printf("  │   if freed memory still has the pointer value, the    │\n");
	printf("  │   app block may appear reachable.                      │\n");
	printf("  │                                                       │\n");
	printf("  │ New tool:     DETECTS LEAK (true positive)            │\n");
	printf("  │   Why? MQ message buffer is freed, no longer in heap   │\n");
	printf("  │   walk. App block has no path from any true root.      │\n");
	printf("  │   Correctly reported as UNREACHABLE.                   │\n");
	printf("  │                                                       │\n");
	printf("  │ KEY DIFFERENCE: This test may show different results   │\n");
	printf("  │ between old and new tools, demonstrating the           │\n");
	printf("  │ improvement in accuracy.                              │\n");
	printf("  └─────────────────────────────────────────────────────┘\n");
	printf("\n");
