...
void test_work_cancel_without_free(void)
{
	...
	printf("  │                                                       │\n");
	printf("  │ Current tool: MAY MISS (false negative likely)        │\n");
	printf("  │   Why? work_s is in BSS (root source). Even after     │\n");
	printf("  │   work_cancel(), the arg field may still contain      │\n");
	printf("  │   the pointer. BSS scan finds it → marks reachable.  │\n");
	printf("  │   Also, freed memory may contain stale pointer.        │\n");
	printf("  │                                                       │\n");
	printf("  │ New tool:     MAY ALSO MISS (same BSS issue)          │\n");
	printf("  │   Why? BSS is still a root source. If work_s.arg      │\n");
	printf("  │   is not cleared by work_cancel(), the stale          │\n");
	printf("  │   pointer will be found during root marking.          │\n");
	printf("  │                                                       │\n");
	printf("  │ IMPORTANT: This test reveals a limitation of BOTH      │\n");
	printf("  │ tools - stale pointers in BSS-rooted structures.       │\n");
	printf("  │ The snapshot diff mode (Mode B) of the new tool       │\n");
	printf("  │ would catch this: the block appears in the \"after\"    │\n");
	printf("  │ snapshot but not the \"before\" snapshot.               │\n");

	printf("  └─────────────────────────────────────────────────────┘\n");
	printf("\n");
	printf("  >>> Now run: mem_leak_checker <<<\n");
	printf("\n");
