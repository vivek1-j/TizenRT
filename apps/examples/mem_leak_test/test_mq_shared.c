...
void test_mq_shared_block(void)
{
	...
	printf("  │   Why? heap_check() scans kernel heap, finds pointer  │\n");
	printf("  │   to app buffer in MQ message. Marks it reachable.    │\n");
	printf("  │                                                       │\n");
	printf("  │ New tool:     NO LEAK (buffer REACHABLE)              │\n");
	printf("  │   Why? Kernel heap scanned as root source. MQ message  │\n");
	printf("  │   buffer found → app block marked reachable.           │\n");
	printf("  │                                                       │\n");
	printf("  │ This test should PASS on both tools.                  │\n");
	printf("  │ It verifies kernel heap scanning works correctly.      │\n");
	printf("  └─────────────────────────────────────────────────────┘\n");
	printf("\n");
