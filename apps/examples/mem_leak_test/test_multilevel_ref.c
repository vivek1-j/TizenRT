...
void test_multilevel_ref(void)
{
	...
	printf("  │   v1: NO LEAK (all heap blocks are roots)            │\n");
	printf("  │   v2: NO LEAK (A found in BSS, B & C via transitive) │\n");
	printf("  └─────────────────────────────────────────────────────┘\n");
	printf("\n");
	...
	printf("\n");
	printf("  NOTE: After testing, clean up with:\n");
	printf("    free(g_ml_root->next->next);\n");
	printf("    free(g_ml_root->next);\n");
	printf("    free(g_ml_root);\n");
	printf("    g_ml_root = NULL;\n");
	printf("\n");
}
