...
void test_reference_cycle(void)
{
	...
	printf("  │                                                       │\n");
	printf("  │ Old tool (v1): NO LEAK reported (FALSE NEGATIVE)     │\n");
	printf("  │   Why? Scans ALL heap blocks as roots.                │\n");
	printf("  │   A contains ptr to B → B marked reachable.            │\n");
	printf("  │   B contains ptr to A → A marked reachable.            │\n");
	printf("  │   Both appear reachable, but neither has a real root. │\n");
	printf("  │                                                       │\n");
	printf("  │ New tool (v2): BOTH A and B reported as LEAKS         │\n");
	printf("  │   Why? Only BSS/DATA/stacks are roots.                │\n");
	printf("  │   Heap blocks scanned only transitively from roots.   │\n");
	printf("  │   Since no root points to A or B, neither is marked.  │\n");
	printf("  └─────────────────────────────────────────────────────┘\n");
	printf("\n");
	printf("  >>> Now run both tools from TASH: <<<\n");
	printf("  TASH> mem_leak\n");
	printf("  TASH> mem_leak_v2\n");
	printf("\n");
