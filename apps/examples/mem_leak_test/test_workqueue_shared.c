...
void test_workqueue_shared_block(void)
{
	...
	printf("  │                                                       │\n");
	printf("  │ Current tool: NO LEAK (buffer REACHABLE)              │\n");
	printf("  │   Why? work_s is in BSS (root). heap_check() or       │\n");
	printf("  │   search_addr() finds arg pointer → marks reachable.  │\n");
	printf("  │                                                       │\n");
	printf("  │ New tool:     NO LEAK (buffer REACHABLE)              │\n");
	printf("  │   Why? BSS scanned as root. work_s found in BSS.      │\n");
	printf("  │   arg pointer found → app block marked reachable.     │\n");
	printf("  │                                                       │\n");
	printf("  │ This test should PASS on both tools.                  │\n");
	printf("  │ It verifies work queue pointer tracking works.         │\n");
	printf("  └─────────────────────────────────────────────────────┘\n");
	printf("\n");
	printf("  >>> Now run: mem_leak_checker <<<\n");
	printf("\n");
	printf("  NOTE: After running the checker, clean up by:\n");
	printf("    work_cancel(HPWORK, &g_work);\n");
	printf("    (The buffer will still need to be freed separately)\n");
	printf("\n");
