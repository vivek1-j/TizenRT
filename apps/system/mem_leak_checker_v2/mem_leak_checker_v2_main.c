...
{
	printf("Usage: mem_leak_v2 [--heap=<name>] [--pid=<value>]\n");
	printf("\nOptions:\n");
	...
	printf("\nExamples:\n");
	printf("  mem_leak_v2                        Check all heaps, summary only\n");
	printf("  mem_leak_v2 --heap=kernel          Check kernel heap only\n");
	printf("  mem_leak_v2 --heap=app1 --pid=-1   Check app1, show all details\n");
}
...
int mem_leak_checker_v2_main(int argc, char **argv)
{
	 ...
	 */
	ret = prctl(PR_MEM_LEAK_CHECKER_V2, getpid(), filter_pid, heap_name);
	if (ret < 0) {
		printf("Fail to launch MEMORY LEAK CHECKER V2.\n");
		return ERROR;
	}
